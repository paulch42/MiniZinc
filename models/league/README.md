...
Two models are presented. In both, the key decision varaible is 'schedule' that determines for each round and each game in a round, the home and away teams for that game. Additionally, 'home_away' determines for each team and each round, whether the team plays at home or away in that round.
